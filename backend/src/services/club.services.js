...
export const joinClub = async (req, res) => {
  ...
  try {
    ...
    if (!user || !club) {
      return res.status(404).json({ message: "Usuario o Club no encontrado" });
    }

    const isAlreadyMember = await user.hasMisClubes(club);
    if (isAlreadyMember) {
      return res
        .status(400)
        .json({ message: "El usuario ya está unido a este club" });
    }
